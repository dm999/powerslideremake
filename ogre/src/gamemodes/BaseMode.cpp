...
void BaseMode::initScene()
{
    ...
    //init multiplayer
    {
        mModeContext.mGameState.setMultiplayerEnabled(mLuaManager.ReadScalarBool("Scene.Multiplayer.Enabled", mModeContext.mPipeline));
        ...
        if(mModeContext.mGameState.isMultiplayerEnabled() && !mModeContext.mGameState.isMultiplayerMaster())
        {
...
void BaseMode::initWorld(const Ogre::Vector3 &gravityVector, const Ogre::AxisAlignedBox &bounds)
{
...
void BaseMode::processCollision(btManifoldPoint& cp, const btCollisionObjectWrapper* colObj0Wrap, const btCollisionObjectWrapper* colObj1Wrap, int triIndex)
{
...
void BaseMode::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
{
...
void BaseMode::postRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
{
    if(mCameraMan->getCameraPositionType() != CameraPosition_Bumper)
