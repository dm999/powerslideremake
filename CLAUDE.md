...
- **Massacre sub-mode** — a deathmatch where the field is built in *batches* from a "Racing grid batches" slider (`GameState::mBatchesMin=1` to `mBatchesMax=25`), gated by `GameState::isMassacreEnabled()`. The opponents slider is ignored; the AI count is always `12 × batches − 1`. At 25 batches this hits the `mAIMax=299` ceiling.
  ...
  - **Countdown timer**: massacre has a configurable time limit (5-30 min, step 5, default 10). The countdown is computed from the player's existing race clock (no new timer object): `remaining = mMassacreTimeLimitMinute * 60 - raceClock`. When time expires (`DeathmatchMode::customFrameRenderingQueuedDo2DUI`) the session ends via the same finish sequence as elimination. The countdown displays on the dash panels reusing the lap-time digit display (`BaseRaceMode::frameRenderingQueued` calls `setRaceTime` with remaining time in MM:SS.XX format). Gated behind `isMassacreEnabled()` — regular deathmatch is unaffected.
    - **Starts only when the player's car is in the race.** The countdown gate is the *per-vehicle* `getPlayerCar().getPhysicsVehicle()->getRaceStarted()` — NOT the game-level `getRaceStarted()`, which turns true at GO while the player (riding the last batch) may still be parked on the grid. Before release the dash shows the full time limit; parked time is never charged against the player.
    - **Base capture**: the race clock starts at GO (lap timers reset in `beforeStartSequence`), but the player may sit parked for seconds. `mMassacrePlayerStartClock` (BaseRaceMode, `-1.0f` == not captured) is captured lazily the first frame the player is in race and subtracted, so the countdown ticks from full limit at release.
    - **Read the raw lap timer, not `LapUtils::mLapTime`.** `getLapTime()` is cached during the physics step (`calcLapTime`), so on the GO frame it still holds the pre-race value while the timer has already been reset — a one-frame-late value. A single-batch launch (player released at GO) therefore captured the wrong base and showed `limit + pre-race_time`. `BaseRaceMode::getPlayerRaceClock()` reads `totalTime + getLapTimer().getMilliseconds() / 1000` (live `PausedTimer`) and is shared by both the HUD display and the session-end check.
    - After the race finishes the race clock freezes (`checkCheckPoints` is guarded on `!getRaceFinished()`), so the same `isMassacreEnabled()` display branch keeps showing the final remaining time through the finish sequence.
  - **Scoring**: at session end `DeathmatchMode::computeDeathmatchScore()` scans all AI vehicles: eliminated (life ≤ 0) × 100 pts + injured (0 < life < 1.0) × 25 pts. Counts and score are stored on `BaseRaceMode` (`mDeathmatchEliminatedCount`, `mDeathmatchInjuredCount`, `mDeathmatchScore`), copied to `ModeContext` on teardown (`GameModeSwitcher`), and displayed below the deathmatch stats table (`UIMainMenuLabels::showDeathmatchStatsLabels`).
  - **Time limit slider** (race options, deathmatch+massacre only): "Time limit (min)" row at Y=202 with left/right buttons, persisted as `massacre time limit` in `player.str`, clamped to `[mMassacreTimeLimitMin=5, mMassacreTimeLimitMax=30]` with `mMassacreTimeLimitStep=5`. Enabled/disabled alongside the batches slider when the Massacre checkbox toggles.

### Shadow system

A single-shadow-map system using Ogre `SHADOWTYPE_TEXTURE_MODULATIVE` with manual PCF sampling in terrain shaders. Key facts for a future editor:

- **Technique**: one 1024×1024 `PF_FLOAT16_R` shadow texture. `setShadowTextureSelfShadow(false)`, `setShadowCasterRenderBackFaces(false)`. Caster material `Test/ShadowCaster` renders normalized depth (z/w + 0.0008 bias) with texel-offset vertex bias.
- **Shadow light**: a dedicated `LT_SPOTLIGHT` named `"shadow_light"` created in `StaticMeshProcesser.cpp:200` and stored on `GameState`. 80° inner / 90° outer cone. Repositioned every frame in `CameraMan::setYawPitchDist` (`.cpp:372`): placed 40 units behind the player car opposite the global light direction, pointing at the car. The Lua config value `Model.ShadowLightDistance` is read in `BaseRaceMode::initScene` but **not** forwarded to `CameraMan` (the 40 is hardcoded).
- **Only one shadow caster**: the player car (`PSPlayerCar.cpp:37` sets `mModelEntity[q]->setCastShadows(gameState.isCastShadows())`). AI car entities are explicitly set to `false` in `PSBaseGraphicsVehicle.cpp:180`. Terrain also has `setCastShadows(false)` (`.cpp:416`). All track lights have `setCastShadows(false)` (`.cpp:155`).
- **Terrain receives shadows manually**: terrain materials use `receive_shadows off` and sample the shadow map via projective texturing (`texviewproj * model * position` in `VertSimpleShadowSoft.glsl`). The fragment shader (`FragSimpleShadowSoft.glsl`) applies 4×4 PCF soft shadows with edge detection (`dxdy()` function) and `shadowDarkness = 0.3`. Shadows fade with distance via `shadowFadeRange = 0.01`. Border mode: white (fully lit) outside the shadow frustum.
- **Vehicle spotlight shadow receiver materials** exist but are **not assigned** to terrain in the current Lua config: `SpotDiffuseShadowSmoothReciever`, `SpotNormalShadowSmoothReciever`, `SpotParallaxOcclusionShadowSmoothReciever` — these use `iteration once_per_light` and darker shadows (0.8–0.9 darkness).
- **Light listeners exclude the shadow spotlight from vehicle lighting**: both `VehicleSceneObjectListener` and `PlayerVehicleSceneObjectListener` `continue` on `LT_SPOTLIGHT` lights, so `shadow_light` never contributes diffuse/specular to cars.
- **No-shadow fallback**: when `isCastShadows` is false, terrain uses `Test/DiffuseSimpleNoShadow` (no shadow-map sampling, just `diffuseMap.rgb * C.rgb`).
- **Lua config** (`assets/scripts/models/vehicle.lua`): `Model.ShadowLightDistance = 40.0`, `Model.ShadowLightOuterAngle = 90.0`, `Model.ShadowLightInnerAngle = 80.0`. (`assets/scripts/scene/Scene.lua`): `Scene.ShadowMapSize = 1024`. Terrain material selection driven by `Terrain.Material.SingleSubMaterial*` keys (`Main.lua`).
- **`AdjustBufferToUseShadow`** (`OgreTools.cpp:367`) is a **misnomer** — it finds vertex indices in the car chassis mesh that match suspension mounting points (for later `AdjustSuspension` animation). It does not deal with shadow mapping.
- **Adding AI car shadows** (non-massacre only): requires restoring `mModelEntity[q]->setCastShadows(gameState.isCastShadows())` in `PSBaseGraphicsVehicle.cpp` (replacing the hard `false`), optionally gated on `!isDeathmatch()` in `PSAICar.cpp` to suppress in massacre. No shader or light changes needed — the existing terrain receiver samples the same shadow map regardless of what cast into it.
