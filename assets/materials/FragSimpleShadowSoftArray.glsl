...
void main()
{
    ...
    //gl_FragColor = vec4(texture2DArray(diffuseMap, T).rgb * (ambient + C.rgb) * shadowColor, 1.0);
    gl_FragColor = vec4(texture2DArray(diffuseMap, T).rgb * C.rgb * shadowColor, 1.0);
}
